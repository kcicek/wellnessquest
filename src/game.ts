import { LoadedData, PlayerState, Food, FitnessAction, Disease } from './types';
...
export class Game {
  ...
  private selectedFitness: FitnessAction | null = null;
  private dayLog: string[] = [];
  private hardshipFactor = 1.0; // 1.0 = normal; >1 harder (more decay, less income), <1 easier
  ...
  getLog(): string { return this.dayLog.join('\n'); }
  pushLog(line: string) { this.dayLog.push(line); }
  resetDayLog() { this.dayLog = []; }

  setHardshipFactor(f: number) {
    if (f < 0.25) f = 0.25; else if (f > 3) f = 3; // clamp safety
    this.hardshipFactor = Number(f.toFixed(2));
    this.pushLog(`Hardship factor set to ${this.hardshipFactor}x`);
  }

  setFood(foods: Food[] | Food) {
  ...
  setFitness(action: FitnessAction) { this.selectedFitness = action; }
  ...
  private applyFoodEffects(food: Food) {
    const s = this.state;
    ...
    if (food.minerals) s.minerals += food.minerals;
    s.money -= food.price;
  }
  ...
  private workPhase() {
    ...
    if (this.state.mood < 25) income *= economy.lowMoodPenalty; else income += income * economy.moodBonus * (this.state.mood / 100);
    const healthyStats = ['vitamins','minerals','protein'].filter(k => (this.state as any)[k] >= 60).length;
    ...
    if (this.state.streakHealthyMeals >= 3 && this.state.fitness >= 70) { income += economy.peakBonus; this.pushLog('Peak condition bonus applied.'); }
    for (const d of this.state.activeDiseases) {
      if (d.effect.incomeMultiplier !== undefined) income *= d.effect.incomeMultiplier;
      if (d.effect.money) income += d.effect.money;
    }
  ...
  private diseaseCheckPhase() {
    const s = this.state;
    const hist = s.history;
    hist.sugar.push(s.sugar); if (hist.sugar.length > 5) hist.sugar.shift();
    hist.cholesterol.push(s.cholesterol); if (hist.cholesterol.length > 5) hist.cholesterol.shift();
    hist.sodium.push(s.sodium); if (hist.sodium.length > 5) hist.sodium.shift();

    const triggered: Disease[] = [];

    for (const disease of this.data.diseases) {
      if (s.activeDiseases.find(d => d.name === disease.name)) continue; // already active
      ...
      if (ok) {
        ...
        if (disease.effect.canWork === false) s.blockedWork = true;
      }
    }
    ...
    for (const d of triggered) {
      ...
      if (d.effect.energy) s.energy += d.effect.energy;
      if (d.effect.mood) s.mood += d.effect.mood;
      if (d.effect.money) s.money += d.effect.money;
    }
    if (triggered.length) {
  ...
  private endOfDay() {
    ...
    for (const [k, v] of Object.entries(decay)) {
      ...
      (this.state as any)[k] += scaled;
    }
    // Apply cross-stat health adjustments before clamping
    this.adjustHealth();
    // Clamp
    const lim = this.data.gameRules.rules.statLimits;
    const clampKeys: (keyof PlayerState)[] = ['energy','fitness','mood','vitamins','minerals','protein','sugar','cholesterol','sodium','health'];
    for (const k of clampKeys) {
      const maxMap: any = { energy: lim.energyMax, fitness: lim.fitnessMax, mood: lim.moodMax, health: lim.healthMax };
      const max = maxMap[k] ?? lim.max;
      (this.state as any)[k] = clamp((this.state as any)[k], lim.min, max);
    }
  ...
  private adjustHealth() {
    const s = this.state;
    let delta = 0;
    const add = (d: number) => { delta += d; };

    // Macro / micro nutrients
    for (const k of ['vitamins','minerals','protein'] as const) {
      const v = (s as any)[k];
      if (v < 20) add(-4); else if (v < 40) add(-2);
    }
    if (['vitamins','minerals','protein'].every(k => (s as any)[k] >= 60)) add(3);

    // Excess markers
    for (const k of ['sugar','cholesterol','sodium'] as const) {
      const v = (s as any)[k];
      if (v >= 110) add(-5); else if (v >= 100) add(-2);
    }

    // Energy & mood low penalties
    if (s.energy < 15) add(-6); else if (s.energy < 25) add(-3);
    if (s.mood < 15) add(-6); else if (s.mood < 25) add(-3);

    // Positive synergy for good condition
    if (s.fitness >= 70 && s.mood >= 60 && s.energy >= 60) add(2);
    if (s.streakHealthyMeals >= 3) add(2);

    // Bound total daily delta
    if (delta < -12) delta = -12; else if (delta > 8) delta = 8;
    if (delta !== 0) {
      s.health += delta;
  ...
  private evaluateDeath() {
    const s = this.state;
    if (s.isDead) return; // already dead
    if (s.health >= 50) return; // no death checks above this threshold
  ...
  private deriveDeathReason(): string {
    const s = this.state;
    // Simple heuristic: pick the worst offending stat extremes
    const issues: string[] = [];
    if (s.sugar > 115) issues.push('Complications from high sugar');
    if (s.cholesterol > 115) issues.push('Cardiovascular failure');
    if (s.sodium > 115) issues.push('Hypertensive crisis');
    if (s.energy < 10) issues.push('Systemic exhaustion');
    if (s.mood < 10) issues.push('Severe depressive collapse');
    if (issues.length === 0) return 'Health failure';
    return issues[0];
  }
}
