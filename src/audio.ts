...
interface PlayOpts { freq?: number; type?: OscillatorType; duration?: number; volume?: number; attack?: number; release?: number; sweepTo?: number; }
...
class AudioManager {
  ...
  init() {
    ...
    try {
      ...
      this.started = true;
      if (this.debug) console.log('[Audio] context started state=', this.ctx.state);
      // iOS unlock silent buffer
      const buffer = this.ctx.createBuffer(1, 1, this.ctx.sampleRate);
      const src = this.ctx.createBufferSource();
      src.buffer = buffer; src.connect(this.masterGain); src.start(0);
  ...
  setEnabled(on: boolean) {
    this.enabled = on;
    if (this.masterGain) this.masterGain.gain.value = on ? 0.6 : 0;
    if (on) this.confirm();
  }
  ...
  private logAttempt(label: string) { if (this.debug) console.log('[Audio] play attempt', label, this.status()); }
  private autoInit() { if (!this.started) { if (this.debug) console.log('[Audio] autoInit attempt'); this.init(); } }

  private ensureContext() {
    if (!this.ctx || this.ctx.state === 'closed') {
      if (this.reinitAttempts < this.maxReinit) {
        this.reinitAttempts++;
        if (this.debug) console.warn('[Audio] context missing/closed – reinitializing attempt', this.reinitAttempts);
  ...
  private prepareFallbackElement() {
    if (this.fallbackAudioEl) return;
  ...
  private fallbackBeep(label?: string) {
    if (!this.fallbackAudioEl) return;
    try {
      this.fallbackAudioEl.currentTime = 0;
      this.fallbackAudioEl.play().then(() => {
        if (this.debug) console.log('[Audio] fallback beep played', label);
      }).catch(err => {
        if (this.debug) console.warn('[Audio] fallback beep failed', err);
      });
    } catch (e) {
      if (this.debug) console.warn('[Audio] fallback beep exception', e);
    }
  }

  installLifecycleHandlers() {
    const resume = () => { try { if (this.ctx && this.ctx.state === 'suspended') { this.ctx.resume(); if (this.debug) console.log('[Audio] visibility/focus resume'); } } catch {} };
  ...
  private startKeepAlive() {
    ...
    this.keepAliveId = window.setInterval(() => {
      ...
      if (this.ctx.state === 'suspended') {
        ...
        if (this.debug) console.log('[Audio] keepAlive resume attempted', this.status());
      }
    }, 4000);
  }

  private playTone(opts: PlayOpts & { __label?: string }) {
    this.autoInit();
    this.ensureContext();
    if (!this.enabled) { if (this.debug) console.warn('[Audio] skip play (disabled)', opts.__label); return; }
    if (!this.ctx || !this.masterGain) { if (this.debug) console.warn('[Audio] skip play (no ctx/masterGain)', opts.__label, this.status()); return; }
    if (this.ctx.state === 'suspended') { this.ctx.resume().catch(()=>{}); if (this.debug) console.log('[Audio] resume before play', this.status()); }
    ...
    try {
      ...
      this.playCount++;
      if (this.debug) console.log('[Audio] play ok', opts.__label, 'count=', this.playCount);
      // If still early (first few plays) and user hasn't heard anything, fallback beep can layer (optional) – only when count < 2
      if (this.playCount < 2) this.fallbackBeep(opts.__label);
    } catch (err) {
      this.lastError = err;
      if (this.debug) console.error('[Audio] play error', opts.__label, err);
  ...
  __bufferTest() {
    this.autoInit(); this.ensureContext();
    if (!this.ctx || !this.masterGain) { if (this.debug) console.warn('[Audio] bufferTest no ctx'); return; }
    try {
      ...
      const src = this.ctx.createBufferSource();
      src.buffer = buffer; src.connect(this.masterGain); src.start();
      this.playCount++;
      if (this.debug) console.log('[Audio] bufferTest ok count=', this.playCount);
    } catch(e) {
      this.lastError = e;
      if (this.debug) console.error('[Audio] bufferTest error', e);
...
;(window as any).audioManager = audioManager; // expose for console debugging
